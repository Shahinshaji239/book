...
export default function PetAct9() {
  ...
  // --- STATE MANAGEMENT ---
  // State from your original PetAct9 file
  ...
  // --- AUTOMATION LOGIC (from GodAct2, adapted for PetAct9) ---
  ...
  useEffect(() => {
    ...
    // ADAPTATION: Using the audio file for Peter Rabbit question 9
    const questionAudio = new Audio('/prob.mp3');
    ...
    questionAudio.onended = () => {
      console.log("DEBUG: Peter question 9 audio finished.");
  ...
  // --- DYNAMIC SUBMIT ANSWER FUNCTION (adapted for PetAct9) ---
  const submitAnswer = async (answer, submissionType) => {
    ...
    try {
      // ADAPTATION: Using the API endpoint for Peter question 9
      ...
      if (submissionType === 'voice') {
        speakText(data.message, () => {
          ...
          // ADAPTATION: Using a placeholder audio for the next prompt
          const nextPromptAudio = new Audio('/prob.mp3');
  ...
  // --- HANDLER FUNCTIONS (adapted for PetAct9) ---
  const handleTextSubmit = () => {
    if (problemAnswer.trim()) {
      submitAnswer(problemAnswer, 'text');
    }
  };
  ...
  const handleProceedToNext = () => {
    stopSpeaking();
    // ADAPTATION: Navigate to the next Peter Rabbit activity page
    navigate('/PetAct10');
  };
  ...
  return (
    <div style={{
      ...
      <style>
        {`
         ...
         @keyframes spin {
          ...
          /* IMPROVED RESPONSIVE DESIGN - SAME AS GODACT2 */
      ...
      <div className="banner-section">
        <img src="/tp9.png" alt="Banner Background" className="banner-img" />

        {showPlayButton && (
          <button className="audio-play-button" onClick={() => new Audio('/prob.mp3').play()}>
            <FaPlay />
          </button>
        )}
      </div>
      ...
      <div className="main-content">
        ...
        <div className="content-with-button">
          ...
          {interactionStage === 'voice' ? (
            ...
            // UI for text input stage (adapted for Peter Rabbit Main Problem)
            <>
              <div className="question-section">
                <h2 className="question-title">What was the main problem in the story?</h2>
                <div className="answer-field">
                  <label className="field-label">Main Problem</label>
                  {!feedback ? (
                    ...
                    <div className="answer-input" style={{ backgroundColor: '#e9ecef', cursor: 'default', minHeight: '55px', padding: '18px 24px' }}>
                      {renderHighlightedText(problemAnswer, feedback.misspelled_words)}
                    </div>
                  )}
                </div>
              </div>
